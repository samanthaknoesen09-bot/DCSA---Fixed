...
export function LiveChatWidget({ isOpen: isOpenProp, onClose: onCloseProp }: LiveChatWidgetProps = {}) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      text: "Hey! I'm Sam from DCSA (Debt Clear SA (Pty) Ltd). Whether you're just curious or seriously stressed about money — either way, you're welcome here. Ask me anything, there are no silly questions. What's on your mind?",
      sender: "bot",
      timestamp: new Date(),
    },
  ])
  ...
  const sendChatTranscript = () => {
    const transcript = messages
      .map(m => `${m.sender === "user" ? "Client" : "DCSA (Debt Clear SA (Pty) Ltd) Bot"}: ${m.text}`)
      .join("\n\n")
