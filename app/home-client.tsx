...
function Hero() {
  return <section className="overflow-hidden" style={{ backgroundColor: colors.blush }}>
    <div className="mx-auto flex max-w-6xl flex-col gap-10 px-6 py-14 md:flex-row md:items-center md:gap-16 md:px-10 md:py-24">
      ...
      <div className="relative flex-1 md:pt-8"><div className="relative mx-auto max-w-md overflow-hidden rounded-[2rem] shadow-xl" style={{ backgroundColor: colors.maroon }}><Image src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/WhatsApp%20Image%202026-09-11%20at%2022.08.00.jpegg-kIsGqKnWQCMzzbogIWdFQae1doHwN4.jpeg" alt="Sam smiling warmly at home" width={1268} height={1268} className="aspect-[4/5] object-cover object-center" priority /><div className="absolute bottom-5 left-5 right-5 rounded-2xl p-4 backdrop-blur-sm" style={{ backgroundColor: "rgba(255,248,247,0.9)" }}><p className="font-serif text-2xl" style={{ color: colors.maroon }}>Let&apos;s fix it together.</p><p className="mt-1 text-sm" style={{ color: colors.mutedText }}>No judgement. No pressure.</p></div></div><div className="mx-auto mt-4 hidden w-fit rounded-2xl px-4 py-3 shadow-lg sm:block" style={{ backgroundColor: colors.warmWhite }}><Heart className="mb-1 h-5 w-5" style={{ color: colors.rose }} /><p className="text-sm font-semibold" style={{ color: colors.maroon }}>No judgement here.</p></div></div>
    </div>
  </section>
}
...
function BreatheAgain() {
  ...
  return <section className="px-6 py-20 md:px-10 md:py-28" style={{ backgroundColor: colors.cream }}><div className="mx-auto max-w-6xl text-center"><p className="text-sm font-semibold uppercase tracking-[0.18em]" style={{ color: colors.rose }}>The other side of debt help</p><h2 className="mt-3 text-balance font-serif text-4xl font-medium md:text-5xl" style={{ color: colors.maroon }}>Imagine breathing again.</h2><p className="mx-auto mt-4 max-w-2xl text-lg leading-relaxed" style={{ color: colors.mutedText }}>Debt help isn&apos;t just about surviving the stress. It&apos;s about getting moments like these back.</p><div className="mx-auto mt-12 grid max-w-5xl gap-8 sm:grid-cols-2 lg:grid-cols-3">{shots.map((s) => <figure key={s.src} className="overflow-hidden rounded-3xl shadow-sm" style={{ backgroundColor: colors.white }}><Image src={s.src} alt={s.caption} width={800} height={600} className="aspect-[4/3] w-full object-cover" loading="lazy" /><figcaption className="min-h-24 px-6 py-5 text-center font-serif text-lg leading-snug" style={{ color: colors.maroon }}>{s.caption}</figcaption></figure>)}</div><div className="mt-10"><WhatsAppButton>I want this — Chat to Sam 💬</WhatsAppButton></div></div></section>
}
