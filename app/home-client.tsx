  ...
  return <section className="overflow-hidden" style={{ backgroundColor: colors.blush }}>
    <div className="mx-auto flex max-w-6xl flex-col gap-10 px-6 py-14 md:flex-row md:items-center md:gap-16 md:px-10 md:py-24">
      <div className="max-w-2xl flex-1">
        ...
        <h1 className="max-w-xl text-balance font-serif text-5xl font-medium leading-[1.04] md:text-7xl" style={{ color: colors.maroon }}>You&apos;re not bad with money. You&apos;re under pressure.</h1>
        <p className="mt-6 max-w-lg text-lg leading-relaxed md:text-xl" style={{ color: colors.charcoal }}>Debt can make every month feel like a race you can&apos;t win. Let&apos;s slow it down, understand what&apos;s happening, and find a way forward that feels like yours.</p>
      ...
      <div className="relative flex-1 md:pt-8"><div className="relative mx-auto max-w-md overflow-hidden rounded-[2rem] shadow-xl" style={{ backgroundColor: colors.maroon }}><Image src={TEAM_IMAGES.samantha} alt="Sam, an NCR registered debt counsellor" width={620} height={740} className="aspect-[4/5] object-cover object-top mix-blend-luminosity opacity-90" priority /><div className="absolute bottom-5 left-5 right-5 rounded-2xl p-4 backdrop-blur-sm" style={{ backgroundColor: "rgba(255,248,247,0.9)" }}><p className="font-serif text-2xl" style={{ color: colors.maroon }}>There is a way through this.</p><p className="mt-1 text-sm" style={{ color: colors.mutedText }}>Let&apos;s find it together.</p></div></div><div className="absolute -bottom-5 -left-2 hidden rounded-2xl px-4 py-3 shadow-lg sm:block" style={{ backgroundColor: colors.warmWhite }}><Heart className="mb-1 h-5 w-5" style={{ color: colors.rose }} /><p className="text-sm font-semibold" style={{ color: colors.maroon }}>No judgement here.</p></div></div>
    </div>
  </section>
}
...
function Location() {
  return <section className="px-6 py-20 md:px-10 md:py-28" style={{ backgroundColor: colors.warmWhite }}><div className="mx-auto flex max-w-5xl flex-col gap-8 rounded-[2rem] p-8 md:flex-row md:items-center md:justify-between md:p-12" style={{ backgroundColor: colors.cream }}><div><Coffee className="h-8 w-8" style={{ color: colors.rose }} /><h2 className="mt-4 font-serif text-4xl font-medium" style={{ color: colors.maroon }}>Come in for a coffee.</h2><p className="mt-3 max-w-xl text-lg leading-relaxed" style={{ color: colors.mutedText }}>If talking face to face feels easier, you&apos;re welcome at Pier 14 in North End. Walk-ins are welcome Monday to Friday, 8am–4pm.</p><div className="mt-5 flex items-start gap-2 text-sm" style={{ color: colors.charcoal }}><MapPin className="mt-0.5 h-4 w-4" style={{ color: colors.rose }} /><span>2nd Floor, Pier 14 Shopping Centre, North End, Gqeberha</span></div></div><div className="shrink-0"><WhatsAppButton>Book a chat</WhatsAppButton><a href="tel:+27719006298" className="mt-4 flex items-center justify-center gap-2 text-sm font-semibold" style={{ color: colors.maroon }}><Phone className="h-4 w-4" />{WHATSAPP_NUMBER}</a></div></div></section>
}
